...
#1B
def Ukuran(matriks):
    return("Ukuran Matriks = "+str(len(matriks))+" x "+str(len(matriks[0])))
...
def Kali(matriks1,matriks2):
    mat3 = []
    if Ukuran(matriks1) == Ukuran(matriks2):
...
def determinan(matriks):
    if len(matriks) == len(matriks[0]):
        bil = [x for x in range(len(matriks))]
        jum = 0
        for i in range(len(matriks)):
            total = 1
            for x in range(len(matriks)):
                total *= matriks[x][bil[x]]
            bil += [bil.pop(0)]
            jum += total
        bil2 = [x for x in range(len(matriks))]
        bil.reverse()
        jum2 = 0
        for i in range(len(matriks)):
            total2 = 1
            for x in range(len(matriks)):
                total2 *= matriks[x][bil2[x]]
            bil2 += [bil2.pop()]
            jum2 += total2
        print(total-total2)
        return ""
...
#2A
def buatNol(m, n):
    matriks = [[0 for x in range(m)] for i in range(n)]
    print(matriks)
